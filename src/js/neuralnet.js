...
import { TEXTURES, OBJ_MODELS, SHADER_CONTAINER } from './loaders'
import { renderer } from './scene'

const DEFAULT_NEURON_COLOR = '#1923c5'
...
export default function NeuralNetwork () {
  ...
  this.neuronColor = DEFAULT_NEURON_COLOR
...
NeuralNetwork.prototype.initNeurons = function (inputVertices) {
  ...
  for (i = 0; i < this.components.neurons.length; i++) {
    this.neuronAttributes.color.value[ i ] = new THREE.Color(DEFAULT_NEURON_COLOR) // initial neuron color
...
NeuralNetwork.prototype.update = function (deltaTime) {
  ...
  for (let ii = 0; ii < this.components.neurons.length; ii++) {
    const n = this.components.neurons[ ii ]
...
NeuralNetwork.prototype.resetAllNeurons = function () {
  this.numPassive = 0
  for (let i = 0; i < this.components.neurons.length; i++) { // reset all neuron state
    const n = this.components.neurons[ i ]
